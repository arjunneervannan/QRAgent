#!/usr/bin/env python3
"""
Hugging Face Jobs runner for Qwen2.5-VL-7B-Instruct factor evaluation.
This script is designed to be run as a Hugging Face Job.
"""
...
try:
    ...
    print(f"Error importing QRAgent_Env: {e}")
    print("Make sure QRAgent_Env is properly installed and accessible")
    sys.exit(1)
...
def main():
    ...
    try:
        from envs.factor_env import FactorImproveEnv
        print("QRAgent_Env imported successfully")
    except ImportError as e:
        print(f"Error importing QRAgent_Env: {e}")
        print("Please ensure QRAgent_Env is properly installed")
        return
    ...
    try:
        ...
        if not os.path.exists(data_path):
            ...
            print("Please ensure the QRAgent_Env data files are present")
            return
